...
def scrape_ebay(brand, product, oem_number=None, asin_number=None):
    ...
    try:
        ...
        with BROWSER_START_LOCK:
            print(f"[{session_id}] Acquire Lock: Starting eBay Browser...")
        ...
        for current_page in range(1, max_pages + 1):
            print(f"[{session_id}] Scraping eBay Page {current_page}...")
            
            url = f"https://www.ebay.com/sch/i.html?_nkw={base_query}&_sacat=0&_from=R40&_trksid=p4432023.m570.l1311&_pgn={current_page}"
            ...
            try:
                ...
                for card in product_cards:
                    url_tag = card.select_one("a.s-item__link") or card.select_one("a.s-card__link")
                    ...
                    junk_words = [
                        r"shop on ebay", r"open in new tab", r"click to see price", r"see price",
                        r"ships\s*(today|free|in\s*\d+\s*days)", r"free shipping", r"sponsored",
                        r"opens in a new window or tab", r"new window or tab", r"^new\s*", r"\bfree\b",
                    ]
                    for junk in junk_words:
                        name = re.sub(junk, "", name, flags=re.IGNORECASE)

                    name = name.strip()
                    if not name or name.lower() == "shop on ebay": continue
        ...
        try:
            save_to_excel("eBay", all_scraped_data)
...
#         for current_page in range(1, max_pages + 1):
#             print(f"Scraping eBay Page {current_page}...")
