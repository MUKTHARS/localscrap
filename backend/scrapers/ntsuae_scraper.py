...
def scrape_ntsuae(brand, product, oem_number=None, asin_number=None):
    ...
    try:
        ...
        with BROWSER_START_LOCK:
            print(f"[{session_id}] Acquire Lock: Starting NTS UAE Browser...")
        ...
        url = f"https://www.ntsuae.com/en/search?text={query}"
        
        print(f"[{session_id}] Scraping: {url}")
        driver.get(url)

        soup = BeautifulSoup(driver.page_source, "html.parser")
        product_cards = soup.select("li.prd-list-item")

        for card in product_cards:
            link_tag = card.select_one('a[title="Product Item"]') or card.select_one("a[href]")
            href = link_tag.get("href", "") if link_tag else ""
            product_url = urljoin("https://www.ntsuae.com", href)

            name_tag = card.select_one("div.prd-title-hov")
        ...
        try:
            save_to_excel("NTSUAE", scraped_data)
        except: pass
...
#             # Product URL
#             product_url = urljoin("https://www.ntsuae.com", (card.select_one('a[title="Product Item"]') or card.select_one("a[href]") or {}).get("href", "")) or "N/A"
