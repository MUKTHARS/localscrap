...
def scrape_amitretail(brand, product, oem_number=None, asin_number=None):
    ...
    try:
        ...
        with BROWSER_START_LOCK:
            print(f"[{session_id}] Acquire Lock: Starting AmitRetail Browser...")
        ...
        for current_page in range(1, max_pages + 1):
            ...
            print(f"[{session_id}] Scraping AmitRetail Page {current_page}...")
            
            try:
                driver.get(url)
                
                if "Page not found" in driver.title:
                    print(f"[{session_id}] Page {current_page} not found. Stopping.")
                    break
                ...
                for card in product_cards:
                    ...
                    currency = "AED"
                    if "Dhs" in raw_price: currency = "Dhs"
                    elif "$" in raw_price: currency = "$"
