...
def get_ticket_breakdown():
    """Get detailed ticket breakdown by urgency and status for stacked bar charts"""
    admin = check_admin_auth()
    if not admin:
        return jsonify({"error": "Unauthorized"}), 401
    ...
    try:
        ...
        breakdown = {
            'critical': {'open': 0, 'in_progress': 0, 'resolved': 0, 'closed': 0},
            'high': {'open': 0, 'in_progress': 0, 'resolved': 0, 'closed': 0},
            'medium': {'open': 0, 'in_progress': 0, 'resolved': 0, 'closed': 0},
            'low': {'open': 0, 'in_progress': 0, 'resolved': 0, 'closed': 0}
        }
        
        # Count tickets by urgency and status
        for ticket in tickets:
            urgency = ticket.urgency
            status = ticket.status
            
            if urgency in breakdown and status in breakdown[urgency]:
                breakdown[urgency][status] += 1
